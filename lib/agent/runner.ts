...
import { answerClarifyQuestion, classifyApprovalWithLLM } from "./approval-gate"
...
export async function* runAgentTurn(opts: RunOptions): AsyncGenerator<AgentEvent> {
  ...
  // Look at graph state to decide: is this a resume from an approval, a
  // clarify-in-place, or a fresh user turn. A pending interrupt shows up as an
  // unfinished task on the last checkpoint.
  ...
  try {
    ...
    if (pendingInterrupts.length > 0) {
      ...
      // Delegate approve/decline/modify/clarify to a small classifier LLM. It
      // sees the pending action, the user's reply, and the last few messages
      // — so "yes, go ahead" reads as approval, "actually make it high
      // priority" reads as modification with concrete guidance, and "when is
      // it due again?" reads as a clarify that the graph doesn't consume.
      yield { type: "progress", label: "Reading your reply…" }
      const recentContext =
      ...
      // CLARIFY — the user asked a question. Keep the interrupt pending: do
      // NOT resume the graph. Emit a short answer generated by the classifier
      // model and stop the turn so the next user message re-classifies.
      if (classification.decision === "clarify") {
        yield { type: "progress", label: "Answering your question…" }
        const answer = await answerClarifyQuestion({
          pendingTool: interruptValue?.tool ?? "unknown",
          pendingArgs: interruptValue?.args ?? {},
          pendingSummary: interruptValue?.summary ?? "",
      ...
      // APPROVE — resume with `{approved: true}`. If the user also asked for
      // something extra, thread the note into feedback so the main agent
      // addresses both the successful write AND the follow-up.
      // DECLINE / MODIFY — resume with `{approved: false, feedback: ...}`.
      // The tools node injects the feedback into the ToolMessage content the
      // main agent reads on its next turn.
      const resumePayload =
        classification.decision === "approve"
          ? classification.note
            ? {
                approved: true as const,
                followUpNote: `After the write succeeds, also address the user's follow-up request: ${classification.note}.`,
              }
            : { approved: true as const }
